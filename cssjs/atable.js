...
aTable.table = function(){
	...
	tdata.prototype.thead=function(){
			...
			for (var key in rowHash) {
				if ($.inArray(key, this.columnSet) == -1){
					...
					if(this.p.colv==undefined){
						...
						var csize='';
						if(this.p.colsize!=undefined){if(this.p.colsize[nocol]!='' && this.p.colsize[nocol]!=undefined){csize=' width="'+this.p.colsize[nocol]+'"';}}
						var calign='';
		...
		if(this.p.colv!=undefined){
			for (var i = 0 ; i < this.p.colv.length ; i++) {
				...
				var csize='';
				if(this.p.colsize!=undefined){if(this.p.colsize[i]!='' && this.p.colsize[i]!=undefined){csize=' width="'+this.p.colsize[i]+'"';}}
				var calign='';
				if(this.p.colalign!=undefined){
	...
	tdata.prototype.sort_by = function(field, ascdsc){
		...
		var primer = function(a){a=a===undefined || a==null?' ':a;return a === parseInt(a,10)?parseInt(a):a.toUpperCase();};
		var key = primer ?
			...
			if (tbl != null) {
				for (var i = 0; i < tbl.rows.length; i++) {
					...
					for (var j = 0; j < tbl.rows[i].cells.length; j++) {
						...
						if(this.p.colhide!=undefined){if(this.p.colhide.indexOf(ncc)>-1){slctmltp.options[ncc].selected=false;cols.push(ncc);}}
