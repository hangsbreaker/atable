...
class Atable
{
	...
	function load()
	{
		...
		$colsize = !empty($this->colsize) ? json_decode($this->colsize, true) : '';
		$colalign = !empty($this->colalign) ? json_decode(strtoupper($this->colalign), true) : '';
	...
	function db_query($qry)
	{
		...
		if ($this->dblink == "mysql") {
			...
			if (!$res) {
				$res = $this->database->_error_message();
			}
		}
		return $res;
	}
				...
				success: function(data){
					...
					forEach.call(atable, function (el, i) {
						if(i==vid[1]){
							...
							document.getElementById("infto"+vid[1]).innerHTML=atblinfto;
							var dtinf=atblinfto.split(" of ");
							if(dtinf[0]==dtinf[1]){
								document.getElementById("loadmore-"+i).style.display="none";
								if(document.getElementById("showall-"+i)){
									document.getElementById("showless-"+i).style.display="inline-block";
